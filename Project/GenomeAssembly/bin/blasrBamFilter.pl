...
my($help, $longest, $coverage, $identity);
$coverage=0.75;
$identity=0.75;
GetOptions(
    'l|longest'=>   \$longest,
    ...
    if(/^@/){
        ...
        if(!defined $readName_1 || $readName_2 ne $readName_1){
            ...
            if(defined $longest){
                ...
                if($myCoverage>=$coverage && $myIdentity>=$identity){
                    my $alignmentValue=join "\t",($_, $myVC, $myVI, $myVS, $myVA);
                    push @outputString, $alignmentValue;
                }
            }
...
sub statisticsCaculation{
    ...
    my $identiy=sprintf("%.4f", ($matchs-$myNM)/$myXL);
    my $similarity=sprintf("%.4f", $matchs/$myXL);
    my $accuracy=sprintf("%.4f", ($matchs-$myNM)/($myXL+$indels));
...
Usage: perl $scriptName *sorted.bam >OUTPUT
    ...
    -i --identiy    FLOAT   Report alignments only if their identiy isn't less than this [defalut: 0.75]
