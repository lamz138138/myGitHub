...
if(myBreaks!='null'){
	outputPlot = outputPlot + geom_histogram(aes(x=data[,myColName], fill=..count..),breaks=myBreaks)
}else if(myBinWidth!='null'){
	outputPlot = outputPlot + geom_histogram(aes(x=data[,myColName], fill=..count..),binwidth=myBinWidth)
}else{
	outputPlot = outputPlot + geom_histogram(aes(x=data[,myColName], fill=..count..),bins=myBins) 
}
#outputPlot = outputPlot + geom_vline(xintercept=5000) + labs(title=myMain, x=myXLab, y=myYLab)
outputPlot = outputPlot + geom_vline(xintercept=5000) + labs(title=myMain, x=myXLab, y=myYLab) + coord_cartesian(xlim = c(0,30000))
